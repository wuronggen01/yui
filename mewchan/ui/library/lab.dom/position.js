$(function() {
    ...
    function getTransformationMatrix(element) 
    {
        ...
        if (element.jqueryPositionMatrix && (new Date().getTime() - element.jqueryPositionMatrix.time.getTime() < 20)) {
            return element.jqueryPositionMatrix.matrix;
        }
    ...
    $.fn.extend(
        {
            "positionToPage": function (position, includingBorder)
                {
                    
                    if (!includingBorder) {

                        var borders = {"left": 0, "top": 0};

                        var computedStyle = window.getComputedStyle(this[0], null);

                        position = {
                            "x": position.x + parseFloat(computedStyle.borderLeftWidth),
                            "y": position.y + parseFloat(computedStyle.borderTopWidth)
                        };

                    }

                    return getTransformationMatrix(this[0]).transformPoint(new Point(position.x, position.y, 0));
                }, 
            "positionFromPage": function (position, includingBorder)
                {

                    var result = getTransformationMatrix(this[0]).inverse().transformPoint(new Point(position.x, position.y, 0));
                    ...
                    if (!includingBorder) {
                        ...
                        return {
                            "x": result.x - parseFloat(computedStyle.borderLeftWidth),
                            "y": result.y - parseFloat(computedStyle.borderTopWidth)
                        };
